...
key_t key = 0x12345678; // 定义消息队列的key
int msq_id;    // 消息队列id
...
int play_and_timeout(int player_id, int timeout);
...
int main(int argc, char *argv[])
{
    ...
    if ((msq_id = msgget(key, IPC_CREAT|IPC_EXCL|0777)) < 0) { // 0777是八进制数字
    ...
    else
    {
        ...
        for (int i = 1; i <= T; i++)
        {
            ...
            // 通知选手出拳并记时， 超时判负(返回-1)
            x = play_and_timeout(1, timeout);
            y = play_and_timeout(2, timeout);
